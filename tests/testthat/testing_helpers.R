...
#'
make_knn <- function(dists, k, decr = TRUE, loops = FALSE) {
  n.row <- nrow(dists)
  n.col <- ncol(dists)
...
#'
create_bigraph_manual <- function(
  cell_dists,
  gene_dists,
  cell_gene_assr,
  gene_cell_assr,
  ...
  cgg_nn <- make_knn(cell_gene_assr, k = k_cg, decr = TRUE, loops = TRUE)
  ...
  ccg_nn <- make_knn(cell_dists, k = k_c, decr = FALSE, loops = loops)

  if (isTRUE(select_genes) & isTRUE(prune_overlap)) {
    overlap_mat <- calc_overlap_deprecated(cc_adj = ccg_nn, cg_adj = cgg_nn)
  ...
  if (!is.null(marker_genes)) {
    ...
    marker_dists <- marker_dists[,
      c(colnames(gene_dists), rownames(marker_dists)),
      drop = FALSE
    ]
    gene_dists <- cbind(
      rbind(gene_dists, marker_dists[, colnames(gene_dists), drop = FALSE]),
      t(marker_dists)
    )
    gene_cell_assr <- rbind(gene_cell_assr, marker_assr)
  }
  ggg_nn <- make_knn(gene_dists, k = k_g, decr = FALSE, loops = loops)

  if (isFALSE(calc_gene_cell_kNN)) {
    gcg_nn <- Matrix::t(cgg_nn)
  } else if (isTRUE(calc_gene_cell_kNN)) {
    gcg_nn <- make_knn(gene_cell_assr, k = k_gc, decr = TRUE, loops = TRUE)
  } else {
