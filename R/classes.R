...
#'
#' @export
setMethod(f = "as.caclust", signature = (obj = "list"), function(obj) {
...
show.caclust <- function(object) {
  ...
  if (!is.empty(cell_clusters(object)) & !is.empty(gene_clusters(object))) {
    ...
    cat("\nClustering results:\n\n")
    df <- data.frame(
      "cluster" = levels(cell_clusters(object)),
...
#' @export
setMethod(f = "rm_monoclusters", signature = (obj = "caclust"), function(obj) {
  ...
  if (!is.empty(obj@bimap)) {
    warning(
      "The biMAP embedding is not valid after removing mono-clusters",
...
#' @rdname rm_monoclusters
#' @export
setMethod(f = "rm_monoclusters", signature = (obj = "Biclust"), function(obj) {
...
setMethod(
  ...
  function(obj, subset_sce = FALSE) {
    if (isFALSE('caclust' %in% names(metadata(obj)))) {
      stop(
        'caclust not found from input SingleCellExperiment object metadata, run caclust function first!'
      )
    }
...
#' @rdname get_caclust
#' @export
setMethod(f = "get_caclust", signature = (obj = "caclust"), function(obj, ...) {
  return(obj)
})
...
setMethod(
  f = "get_caclust",
  signature = (obj = "SingleCellExperiment"),
  function(obj, ..., caclust_meta_name = "caclust") {
